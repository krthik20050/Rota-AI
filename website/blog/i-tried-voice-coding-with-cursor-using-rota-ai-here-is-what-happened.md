...
description: "Voice coding sounds crazy. I tried dictating code into Cursor using Rota AI. It actually worked for some things, failed at others. Here is the honest breakdown."
...
So I opened Cursor, activated Rota AI, and just started talking.

The first thing I said was: write a function that takes a string and returns the number of words.

Cursor wrote it. In Python. Correct indentation and everything.
...
camelCase is bad enough. snake_case makes it worse. Now mix in brackets, parentheses, curly braces, and the occasional backslash. Your mouth was not designed for this.

Variable names are another nightmare. Imagine saying "user_auth_token_expiration_timestamp" out loud. Then imagine your voice tool hearing "user auth token expiration time stamp nd" and inserting an extra word into your variable name that breaks everything.
...
The brackets thing? Still imperfect. But Rota AI is smart enough to keep parentheses paired and indentation mostly correct. It knows you are in a code editor and adjusts.

Is it perfect? No. But it is way better than dictating code into Google Docs and copying it over, which is what I used to do. Shoutout to everyone who has suffered through that workflow. You know the pain.
...
**Comments.** This is the big one. I just say "explain that this function validates the user token and throws an error if it is expired" and a proper comment appears. No stopping to type. No breaking my flow. Just talking through the logic.

**Docstrings.** Same thing. Describe what the function does out loud and a docstring shows up. It is not going to win a writing award but it is accurate and it took three seconds.

**Function descriptions.** "Create a helper that reads a JSON file, parses the config, and returns a default if the key is missing." Boom. Function skeleton done. I fill in the gaps manually.

**Commit messages.** Oh man. I used to write "fix stuff" and "update thing" and "not sure what changed" for my commits. Now I just say "refactor the voice activity detection loop to use a sliding window instead of fixed chunks" and it types that out. My git history looks like a real project now.
...
**Precise variable naming.** I tried to name a variable `max_retry_attempts_backoff_multiplier`. The voice tool heard "max retry attempts back off multiplier" and I spent ten minutes debugging why my code could not find a variable that did not exist. It was called "backoffmultiplier" all one word. Classic.

**Refactoring.** "Move this function from line forty seven to after the utility class and rename it to use camelCase instead of snake." Nope. Just do it manually.
...
Here is what I actually do now. It is and a hybrid approach.

First, I think out loud. I describe what I want the function to do, what parameters it takes, what it returns. Rota AI transcribes and injects that as a comment above where I am working. Think of it like rubber duck debugging but the duck writes things down.
...
your mileage may vary depending on how clear your speech is and how loud your environment is. My roommate walking in mid-function absolutely ruins the flow.
...
I write more comments now. By a lot. Because it is effortless to just explain what something does instead of crafting it through keystrokes. My codebase is more documented than it has ever been and I did not have to sacrifice development speed.
...
For writing actual logic, no. You are faster with a keyboard. For comments, docstrings, and boilerplate, yes. It depends on what kind of code you write.

**What microphone do I use?**

The mikijo blue yeti. But any decent mic works. I tried with laptop and built-in mic and it was terrible. Do yourself favor and get something with noise cancellation.

**Does it work with languages other than Python?**

I have tried JavaScript and TypeScript. Works about the same. I have not tried C++ but I imagine dictating template syntax would be an exercise in suffering.
...
If you are curious, grab Rota AI, fire up Cursor, and try dictating a function out loud. Start with comments. Work your way up to docstrings. See how it feels.

It is not magic. It is and not going to replace your keyboard. But for those midnight coding sessions when your fingers are tired and your brain is still going, it is a pretty solid tool to have in the rotation.
